...
def register():
   ...
   if form.validate_on_submit():
       hashed_pass=generate_password_hash(form.password.data,method="pbkdf2:sha256:600000",salt_length=8)
       try:
           new_user=Users(email=form.email.data,password=hashed_pass,username=form.username.data)
           ...
           flash("You have already registed! Please login.","error")
           print(error)
...
def show_post(post_id):
   ...
   if form.validate_on_submit():
       if not current_user.is_authenticated:
           flash("Only Logged in users can comment")
           return redirect(url_for('login'))
